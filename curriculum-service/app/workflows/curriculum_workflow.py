...
class CurriculumWorkflow:
    ...
    async def interviewer_node(self, state: CurriculumState) -> dict:
        current_profile = state.get("user_profile", {})
        current_goal = state.get("target_goal")
        try:
            result = await self._llm_service.interview(
                [
                    SystemMessage(
                        content=(
                    ...
                    HumanMessage(
                        content=(
                            ...
                            f"대화:\n{self._conversation_text(state['messages'])}"
                        )
                    ),
                ]
            )
        except self._llm_service.structured_output_exceptions:
            logger.exception(
        ...
        missing_info = self._missing_profile_info(merged_profile, target_goal)
        ...
        if missing_info:
            question = result.next_question or self._question_for(missing_info[0])
            update.update(messages=[AIMessage(content=question)], status="interviewing")
        return update

    async def retrieve_node(self, state: CurriculumState) -> dict:
            ...
            normalized_steps.append(
                {
                    ...
                    "reason": (
                        ...
                        if generated
                            ...
                            "앞 단계에서 익힌 내용을 자연스럽게 확장할 수 있는 강의입니다."
                        )
                    ),
                }
            )
    ...
    def _fallback_curriculum(self, state: CurriculumState) -> dict:
        ...
        return {
            "summary": (
                f"{target_goal} 달성을 위해 기초부터 심화까지 단계적으로 구성했습니다."
            ),
            "steps": steps,
        }

    async def planner_node(self, state: CurriculumState) -> dict:
        try:
            plan = await self._llm_service.create_plan(
                [
                    SystemMessage(
                        content=(
            ...
            curriculum = self._normalize_plan(plan, state["retrieved_courses"])
        except self._llm_service.structured_output_exceptions:
            logger.exception(
    ...
    async def feedback_node(self, state: CurriculumState) -> dict:
        latest_message = str(state["messages"][-1].content)
        try:
            result = await self._llm_service.classify_feedback(
                [
                    SystemMessage(
                        content=(
    ...
    def build(self):
        ...
        graph_builder.add_node("feedback", self.feedback_node)
