// curriculum-service/main.py 의 COURSES 12건을 그대로 복제한 임시 카탈로그.
//
// 봇 응답의 step 에는 title·duration 이 이미 들어 있으므로 이 카탈로그는 보조 역할이다.
//   1) 카드에 카테고리를 덧붙인다 (step 에 없는 정보)
//   2) 목 클라이언트가 시나리오 응답을 만들 때 재료로 쓴다
//
// 봇이 course-service 실데이터로 전환되면 course_id 가 숫자(30001~)로 바뀌고
// 여기 있는 marketing-* 3건은 사라진다. course-service 의 Category enum 에
// 마케팅 계열이 없기 때문이다. 그래서 조회는 반드시 폴백을 거친다.
...
const BY_ID = new Map(COURSE_CATALOG.map((course) => [course.id, course]));

/** 카탈로그에 없는 id 면 undefined. 호출부는 카드가 깨지지 않게 처리해야 한다. */
